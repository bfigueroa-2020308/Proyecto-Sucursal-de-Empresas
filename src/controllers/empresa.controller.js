    ...
    try{ 
        ...
        if(!empresas){
            const empresa = await new Empresa({
                nombre :'SuperAdmin',
                contraseña : bcrypt.hashSync('123456'),
                role: 'ADMIN'
            })
            empresa.save();
        }else{
...
exports.agregarEmpresa = async(req,res)=>{
    try{
        ...
        if(!msg){
            ...
            if(!mismoNombre){ 
                data.contraseña = bcrypt.hashSync(params.contraseña);
...
exports.login = async(req,res)=>{
    try{
        ...
        if(!msg){
            ...
            const verificador = await bcrypt.compareSync(params.contraseña, empresaEncontrada.contraseña);
            if(empresaEncontrada && verificador==true){
                const token = await jwt.createToken(empresaEncontrada);
                return res.send({token, message:'Loggeado satisfactoriamente'});
            }else{
        ...
        if(!msg){
            ...
            if(existProducto && existSucursal){
                if(existProducto.stock > params.stock){
                    const existProductoSucursal = await ProductoSucursal.findOne({nombre:existProducto.nombre});
